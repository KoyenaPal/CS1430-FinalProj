...
def Attention_Matrix(K, Q, use_mask=False):
        ...
        # OK so, tensorflow interprets this as matrix multiplication on each batch separately. Or like, parallel across the batch dimension.
        # Also, this IS doing all the masking work. It's not in the transformer block or attenvis or whatever.

        embedding_size = K.shape[2]
        prod = tf.linalg.matmul(Q, K, transpose_b=True)
        prod_norm = tf.divide(prod, tf.math.sqrt(tf.cast(embedding_size, tf.float32)))

        if use_mask: prod_masked = tf.add(prod_norm, atten_mask)
        else: prod_masked = prod_norm

        return tf.nn.softmax(prod_masked)
...
class Atten_Head(tf.keras.layers.Layer):
        def __init__(self, input_size, output_size, use_mask):          
                ...
                self.w_K = self.add_weight("w_K",shape=[input_size, output_size])
                self.w_V = self.add_weight("w_V",shape=[input_size, output_size])
                self.w_Q = self.add_weight("w_Q",shape=[input_size, output_size])
        ...
        def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
                ...
                K = tf.linalg.matmul(inputs_for_keys, self.w_K)
                V = tf.linalg.matmul(inputs_for_values, self.w_V)
                Q = tf.linalg.matmul(inputs_for_queries, self.w_Q)
